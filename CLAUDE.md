...
| D-3 | один менеджер пакетов — **npm**, как в CI | в работе: Claude | Удалён `pnpm-lock.yaml`. Из `package.json` убран блок `pnpm.overrides`: npm его игнорировал, а в `package-lock.json` и так одна `vite 6.3.5`. `.claude/launch.json` → npm. Добавлен `.npmrc` с `legacy-peer-deps=true` — как `npm ci --legacy-peer-deps` в CI. `node_modules` переставлены через `npm ci`, идёт прогон typecheck/test/lint/build |
| ERR-1, D-4 | сделать | **поручено MiMo** | Пробные задачи, проверяет Claude — см. «Задание для MiMo №1» ниже |
