...
3. **Ветки.** Работа — в `mimo/<тема>` или `claude/<тема>`, латиницей. CI проверяет эти
   ветки, но на сайт не выкладывает. В `main` без ревью не пушить: `main` = живой сайт.
...
6. **Одна ветка — одна тема.** Маленькие изменения, которые можно проверить за раз.
7. **Разные папки.** Два агента не правят одну рабочую папку одновременно: переключение
   ветки у одного меняет файлы под другим. MiMo работает в своей копии репозитория
   (`git worktree add` или отдельный `git clone`), желательно вне OneDrive — внутри
   OneDrive mimo падает с `EEXIST ... .git\info`.
...
| S-7 | нет лимита на `/backup/verify` | предложено | Не проверено |
| D-2/3/4 | имя пакета, второй lock-файл, `@types/leaflet` в dependencies | предложено | Не проверено. `pnpm-lock.yaml` после удаления MUI устарел — CI его не использует |
