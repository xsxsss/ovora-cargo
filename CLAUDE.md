...
| ID | Что | Статус | Доказательство / решение |
|---|---|---|---|
| BAK-1 | `/auth/backup/generate` без авторизации и лимита | предложено (Claude) | `index.ts:6246` — без middleware. `handleGenerateBackup` берёт email из тела запроса, перезаписывает резервный код и **на каждый вызов шлёт человеку письмо** и пишет уведомление. Любой без входа может засыпать письмами любого пользователя и выжечь почтовую квоту. Взлома аккаунта нет — код ничего не открывает. Фронтенд эти эндпоинты не использует → предлагаю удалить все три (`generate`, `verify`, `exists`); если фича нужна — требовать `X-User-Token` и лимит. Решает владелец |
