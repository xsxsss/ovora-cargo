...
| Rybbit (аналитика) | ждём `siteId` от владельца | Аккаунт создан; нужен ID сайта из панели rybbit.com. Подключение: `<script src="https://app.rybbit.io/api/script.js" data-site-id="…" async>` в `index.html` + `https://app.rybbit.io` в `script-src` и `connect-src` CSP (`index.html`, `netlify.toml`, `vercel.json`); админку не считать (`data-skip-patterns`) |
