...
| Яндекс.Метрика (аналитика) | ждём номер счётчика от владельца | Rybbit удалён: в облаке платный. Метрика подключена в `config/yandexMetrika.ts` (`675dc07`), включится, когда в `.github/workflows/deploy-pages.yml` появится `VITE_YANDEX_METRIKA_ID` (не секрет). Новые страницы с перепиской, документами, телефонами или платежами — оборачивать в `hiddenFromWebvisor` в `routes.tsx` |
