...
const ACTION_LABELS: Record<string, string> = {
  ...
  'offer.admin_status_change': '🔁 Смена статуса оферты',
  'review.admin_delete': '🗑 Удаление отзыва',
  'document.admin_status_change': '🛂 Проверка документа',
  'settings.admin_update': '⚙ Изменение настроек',
  'user.admin_status_change': '🚫 Блокировка/разблокировка',
  'user.admin_delete': '🗑 Удаление пользователя',
  'blacklist.admin_remove': '✅ Снятие из чёрного списка',
  'trip.admin_delete_all': '🗑 Массовое удаление поездок',
...
const PAGE_SIZE = 50;
const theme = PLATFORM_THEME.cargo;

export function CargoAuditLog() {
  ...
  return (
    <div className="space-y-5">
      ...
      <form onSubmit={handleSearch} className="bg-white rounded-2xl p-4 flex flex-col sm:flex-row sm:flex-wrap gap-3" style={{ border: '1px solid #f0f4f8' }}>
        ...
        <button type="submit" className="w-full sm:w-auto px-4 py-2.5 rounded-xl text-sm font-semibold text-white" style={{ background: theme.accent }}>
          Найти
        </button>
      </form>
          ...
          <div className="divide-y divide-gray-50">
            {entries.map(entry => (
              <div key={entry.id} className="flex items-start gap-3 px-3 sm:px-5 py-3">
                <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 text-base" style={{ background: theme.bg }}>
